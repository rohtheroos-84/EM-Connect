...
func (c *Consumer) Connect() error {
	...
	// Declare the main queue (idempotent — safe even if Spring Boot already created it)
...
// setupQueue declares the main consumption queue with the same args as Spring Boot
func (c *Consumer) setupQueue() error {
	// Declare the events exchange (idempotent)
	err := c.channel.ExchangeDeclare(
		"em.events", "topic", true, false, false, false, nil,
	)
	if err != nil {
	...
	// Bind queue to exchange — ticket queue gets registration.confirmed only
	err = c.channel.QueueBind(
		c.config.RabbitMQ.Queue, "registration.confirmed", "em.events", false, nil,
	)
	if err != nil {
		return fmt.Errorf("failed to bind queue: %w", err)
	}

	log.Printf("📦 Queue declared and bound: %s → em.events (registration.confirmed)", c.config.RabbitMQ.Queue)
	return nil
}
...
func (c *Consumer) setupDLQ() error {
	...
	err = c.channel.QueueBind(
		c.config.RabbitMQ.DLQQueue, "ticket.failed", c.config.RabbitMQ.DLQExchange, false, nil,
	)
	if err != nil {
...
func (c *Consumer) sendToDLQ(msg amqp.Delivery, errorMsg string) {
	...
	err := c.channel.Publish(
		c.config.RabbitMQ.DLQExchange, "ticket.failed", false, false,
		amqp.Publishing{
